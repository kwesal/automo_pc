...
int main(void)
{
	...
	if (sgdma_tx_dev == NULL)
	{
		...
		alt_printf("Opened scatter-gather dma transmit device\n");
	...
	if (sgdma_rx_dev == NULL)
	{
		...
		alt_printf("Opened scatter-gather dma receive device\n");
	...
	*(tse + 2) = *(tse + 2) | 0x0000004B;
	...
	// Packet Sending Program //
	usleep(1000000); //wait hw init

	volatile int *base = (int *)0x00103000; //base address of Power Emulator module
	unsigned int result1 = 0x00000000;		//result buffer

	alt_printf("Program Started!\n");	  //program start
	char new_char;
	int select = 0;
	int counter = 0;

	//while(1){
		alt_printf("Insert cycle: ");
		while ((new_char = alt_getchar()) != '\n')
		{
			alt_printf("%c", new_char);
			select *= 10;
			select += (int)new_char - 48;
		}
		alt_printf("\n0x%x\n", select);
		alt_printf("Start!!! \n\n");
		IOWR_32DIRECT(base, 0x00, 0x00000001); //start control input
		while (1)
		{
			
			if (result1 != *(int *)(0x00103000 + 0x04))
			{
				result1 = *(int *)(0x00103000 + 0x04);
				//result2 = *(int*)(0x00103000+0x08);
				//alt_printf("result %h = %x\n", counter++, result1);
				...
				tx_frame[20] = result1 >> 24;
				tx_frame[21] = (result1 - (tx_frame[20] << 24)) >> 16;
				tx_frame[22] = (result1 - (tx_frame[20] << 24) - (tx_frame[21] << 16)) >> 8;
				tx_frame[23] = (result1 - (tx_frame[20] << 24) - (tx_frame[21] << 16) - (tx_frame[22] << 8));
				tx_frame[24] = '\0';
				...
				//alt_printf("- packet sent\n\n");

				tx_frame[16] = tx_frame[17] = tx_frame[18] = tx_frame[19] = 0;
				tx_frame[20] = tx_frame[21] = tx_frame[22] = tx_frame[23] = 0;
				if (select == counter)
					break;
				counter++;
			}
		...
		alt_printf("finish!!! \n\n");
		IOWR_32DIRECT(base, 0x00, 0x00000002); //start control input
	//}
